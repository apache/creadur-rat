 ...
 */
public class AntOption extends AbstractOption<AntOption> {
    /** Default values for CLI options */
    public static final UnmodifiableMap<Arg, String> DEFAULT_VALUES;
    /** Set of CLI options that are not supported by Ant. */
    ...
    /** Filter to remove options not supported by Ant. */
    static final Predicate<Option> ANT_FILTER;
    /** The filter that matches unsupported options */
    static final Predicate<Option> UNSUPPORTED_FILTER;
    /** The filter that matches options that are mapped to other options */
    static final Predicate<Option> MAPPED_FILTER;
    /** The filter that removes ant options that are mapped to other options */
    static final Predicate<AntOption> UNMAPPED_ANTOPTION_FILTER;

    /** The OptionFactory configuration for AntOptions. */
    public static final OptionFactory.Config<AntOption> FACTORY_CONFIG;
    ...
    static {
        ...
        renameMap.put("addLicense", "add-license");
        RENAME_MAP = (UnmodifiableMap<String, String>) UnmodifiableMap.unmodifiableMap(renameMap);
        ...
        Set<Option> unsupportedSet = Arg.getOptions(ADDITIONAL_OPTIONS).getOptions()
                .stream().filter(o -> Objects.isNull(o.getLongOpt()))
                .collect(Collectors.toSet());
        unsupportedSet.addAll(Arg.LOG_LEVEL.group().getOptions());
        unsupportedSet.addAll(Arg.DIR.group().getOptions());
        unsupportedSet.addAll(Arg.SOURCE.group().getOptions());
        unsupportedSet.addAll(Arg.HELP_LICENSES.group().getOptions());
        UNSUPPORTED_SET = (UnmodifiableSet<Option>) UnmodifiableSet.unmodifiableSet(unsupportedSet);
        ...
        DEFAULT_VALUES = (UnmodifiableMap<Arg, String>) UnmodifiableMap.unmodifiableMap(new HashMap<Arg, String>());
        /*
        ...
        Set<Option> filteredOptions = new HashSet<>(UNSUPPORTED_SET);
        ...
        FACTORY_CONFIG = new OptionFactory.Config<>(AntOption.ANT_FILTER, AntOption::new, AntOption.ADDITIONAL_OPTIONS);
        UNSUPPORTED_FILTER = UNSUPPORTED_SET::contains;
        MAPPED_FILTER = ANT_CONVERSION_MAP.keySet()::contains;
        UNMAPPED_ANTOPTION_FILTER = antOption -> UNSUPPORTED_FILTER.or(MAPPED_FILTER).negate().test(antOption.getOption());
    }
