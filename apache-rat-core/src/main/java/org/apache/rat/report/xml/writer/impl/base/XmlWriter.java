...
public final class XmlWriter implements IXmlWriter {
	...
	public IXmlWriter openElement(final CharSequence elementName)
		...
		if (elementsWritten && elementNames.isEmpty()) {
			throw new OperationNotAllowedException(
					"Root element already closed. Cannot open new element.");
		}
		if (!isValidName(elementName)) {
			throw new InvalidXmlException("'" + elementName
	 ...
	 * directly after {@link #openElement(CharSequence)} or {@link #attribute}.
	 * 
	 * @param name
	...
	public IXmlWriter attribute(CharSequence name, CharSequence value)
		...
		if (!isValidName(name)) {
			throw new InvalidXmlException("'" + name
					+ "' is not a valid attribute name.");
		}
		if (!inElement) {
			throw new InvalidXmlException(
					"Attributes can only be written in elements");
		}
		if (currentAttributes.contains(name)) {
			throw new InvalidXmlException(
					"Each attribute can only be written once");
		}
		writer.write(' ');
		rawWrite(name);
		writer.write('=');
	...
	public IXmlWriter content(CharSequence content) throws IOException {
		if (elementNames.isEmpty()) {
			if (elementsWritten) {
				...
				throw new OperationNotAllowedException(
						"An element must be opened before content can be written.");
			}
		}
		if (inElement) {
			writer.write('>');
	 ...
	 * and closed. No exception is raised when called upon a document whose root
