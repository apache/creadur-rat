...
public abstract class RatTask extends DefaultTask implements RatTaskProperties {
  ...
  public static void setupRatTaskConfigurations(
    ...
    var xmlReportElements = format("%sXmlReportElements", ratTask.getName());
    var xmlReportElementsConfig =
        project
            .getConfigurations()
            .register(
                xmlReportElements,
                c -> {
                  ...
                  c.outgoing(
                      configurationPublications -> {
                        configurationPublications.artifact(ratTask.flatMap(RatTask::getRatXmlFile));
                      });
                });

    var allReportsElements = format("%sAllReportsElements", ratTask.getName());
    var allReportsElementsConfig =
        project
            .getConfigurations()
            .register(
                allReportsElements,
                c -> {
                  ...
                  c.outgoing(
                      configurationPublications -> {
                        for (var reportFileFunction :
                            List.<Transformer<RegularFileProperty, RatTask>>of(
    ...
    for (var reportFileFunction :
        List.<Function<RatTask, RegularFileProperty>>of(
    ...
    var xmlReport = format("%sXmlReport", ratTask.getName());
    project
        .getConfigurations()
        .register(
            xmlReport,
    ...
    var allReports = format("%sAllReports", ratTask.getName());
    project
        .getConfigurations()
        .register(
            allReports,
    ...
    dependencies.add(
        ...
        dependencies.project(
            Map.of("path", project.getPath(), "configuration", xmlReportElements)));
    dependencies.add(
        allReports,
        dependencies.project(
            Map.of("path", project.getPath(), "configuration", allReportsElements)));
  }
  ...
  @TaskAction
  public void check() {
    var workQueue = getWorkerExecutor().classLoaderIsolation();
