...
public abstract class RatWorkAction implements WorkAction<RatWorkParameters> {
  ...
  public void execute() {
    ...
    var ratArguments = new RatOptionsToConfiguration(getParameters());

    var config = getConfiguration(ratArguments);
    ...
    if (config.getClaimValidator().hasErrors()) {
      ...
      if (!config
        ...
        try {
          ...
          LOGGER.error(buffer.toString(StandardCharsets.UTF_8));
        } catch (Exception e) {
  ...
  protected ReportConfiguration getConfiguration(final RatOptionsToConfiguration ratArguments) {
    try {
      var basedir = getParameters().getProjectBaseDir().getAsFile().get();
